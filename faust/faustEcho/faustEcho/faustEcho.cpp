...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "faustEcho.dsp");
		m->declare("name", "faustEcho");
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("faustEcho");
		ui_interface->addHorizontalSlider("del", &fHslider1, FAUSTFLOAT(2e+04f), FAUSTFLOAT(1.0f), FAUSTFLOAT(3e+04f), FAUSTFLOAT(1.0f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow1 = int(std::min<float>(3e+04f, std::max<float>(0.0f, float(fHslider1)))) + 1;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[IOTA0 & 32767] = float(input0[i0]) + fSlow0 * fRec0[(IOTA0 - iSlow1) & 32767];
			output0[i0] = FAUSTFLOAT(fRec0[IOTA0 & 32767]);
			fRec1[IOTA0 & 32767] = float(input1[i0]) + fSlow0 * fRec1[(IOTA0 - iSlow1) & 32767];
			output1[i0] = FAUSTFLOAT(fRec1[IOTA0 & 32767]);
			IOTA0 = IOTA0 + 1;
		}
	}
	...
	#define FAUST_FILE_NAME "faustEcho.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("del", fHslider1, 2e+04f, 1.0f, 3e+04f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("feedback", fHslider0, 0.5f, 0.0f, 1.0f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, del, "del", fHslider1, 2e+04f, 1.0f, 3e+04f, 1.0f) \
		p(HORIZONTALSLIDER, feedback, "feedback", fHslider0, 0.5f, 0.0f, 1.0f, 0.01f) \
...
faustEcho::faustEcho() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void faustEcho::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void faustEcho::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float faustEcho::getParamValue(const std::string& path)
{
